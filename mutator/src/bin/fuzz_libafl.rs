...
fn runc_rootfs_seeds(bin_true: &[u8]) -> Vec<FsDelta> {
    ...
    seeds.push(FsDelta::new(vec![FsOp::create_file(
        "/rootfs/.dockerenv",
        b"".to_vec(),
    )]));
    seeds.push(FsDelta::new(vec![
    ...
    seeds.push(FsDelta::new(vec![FsOp::set_times(
        "/rootfs/bin/true",
        0,
        0,
        0,
...
unsafe fn populate_runc_rootfs(vfs: *mut VfsT, bin_true: &[u8]) {
    ...
    vfs_create_file(
        vfs,
        c"/rootfs/etc/resolv.conf".as_ptr(),
        resolv.as_ptr(),
        resolv.len(),
    );
}
...
fn main() {
    ...
    let campaign = args
        .get(1)
        .map(|s| Campaign::from_arg(s))
        .unwrap_or(Campaign::Foobar);
    ...
    let mut harness = |input: &FsDelta| -> ExitKind {
        ...
        match campaign_copy {
            ...
            Campaign::Libarchive => {
                #[cfg(has_libarchive)]
                {
                    unsafe { fuzz_libarchive_from_path(fuse_input_ptr) };
                    ExitKind::Ok
                }
                #[cfg(not(has_libarchive))]
            ...
            Campaign::Runc => {
                ...
                if let Some(sig) = output.status.signal() {
                    if [libc::SIGSEGV, libc::SIGABRT, libc::SIGBUS, libc::SIGFPE].contains(&sig) {
    ...
    if campaign == Campaign::Runc {
        ...
        println!(
            "  crashes:  runc SIGSEGV/SIGABRT → saved to solutions_{}/\n",
            campaign.name()
        );
    }
