...
use fs_mutator::{
    ...
    ffi::{
        apply_delta, cp_vfs_checksum, enumerate_vfs_all_paths, enumerate_vfs_dir_paths,
        enumerate_vfs_file_paths, vfs_create, vfs_create_file, vfs_destroy, vfs_mkdir,
    ...
    mutators::{
        ...
        ReplaceFileContent, SpliceDelta, UpdateExistingFile, MAX_LIVE_CORPUS, MAX_OPS,
    },
};
    ...
    fn can_apply(&self, d: &FsDelta) -> bool {
        ...
        match self {
            ...
            Self::Replace(_) => d
                .ops
                .iter()
                .any(|o| matches!(o.kind, FsOpKind::CreateFile | FsOpKind::UpdateFile)),
...
fn build_mutator_pool(
    ...
    vec![
        ...
        AnyMutator::MutatePath(MutatePath::with_baseline(file_paths.clone(), dir_paths.clone(), all_paths.clone())),
...
fn main() {
    ...
    for i in 0..n_iters {
        ...
        for j in 0..n_mut {
            ...
            if j == 0 {
                first_name = mutators[m_idx].name();
            }
                ...
                if post_checksum != baseline_checksum {
                    ...
                    if seen_checksums.insert(post_checksum) {
                        ...
                        if corpus.len() < MAX_LIVE_CORPUS {
                            ...
                            let victim =
                                seed_count + state.rand_mut().below(nz(corpus.len() - seed_count));
        ...
        if reset_ret == 0 {
            n_reset_ok += 1;
            reset_total += elapsed;
            if elapsed < reset_min {
                reset_min = elapsed;
            }
            if elapsed > reset_max {
                reset_max = elapsed;
            }
        } else {
            n_reset_err += 1;
    ...
    let reset_min_ns = if n_reset_ok > 0 {
        reset_min.as_nanos()
    } else {
        0
    };
    let reset_max_ns = if n_reset_ok > 0 {
        reset_max.as_nanos()
    } else {
        0
    };
    ...
    println!("  promoted       : {n_promoted}  (novel-checksum deltas pushed to live corpus)",);
    ...
    assert_eq!(
        n_reset_err, 0,
        "VFS reset failures detected — stale state bug"
    );
